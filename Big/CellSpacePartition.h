...
public:
	...
	CellSpacePartition(double width,        //width of the environment
		...
		int   MaxNeighbors);  //maximum number of neighbors to target position

	//adds entities to the class by allocating them to the appropriate cell
	inline void AddEntity(const entity& ent);
	
	//deletes entity from the cell, 如果元素Pos不正确将无法删除，无错误提示
	inline void DeleteEntity(const entity& ent);

	//update an entity's cell by calling this from your entity's Update method 
	inline void UpdateEntity(const entity& ent, const Vector2D& OldPos);
...
m_iNumCellsX(cellsX),
m_iNumCellsY(cellsY),
m_Neighbors(MaxNeighbors, entity())
{
...
//----------------------- DeleteEntity --------------------------------------
//
//  Used to delete the entity from the data structure
//------------------------------------------------------------------------
template<class entity>
inline void CellSpacePartition<entity>::DeleteEntity(const entity &ent)
{ 
	assert (ent);
	//assert(ent->isAlive == true);
	...
	m_Cells[idx].Members.remove(ent);		//如果没有对应元素，不会报错
}
